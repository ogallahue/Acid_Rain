	...
	if(First){  
		...
		if(slope < 0){
			while(ycord > ey){
				...
				Bounce_List.push({xcord,ycord,slope});
			}
		}else{
...
function circleBoundary(cX,cY,r){ //a+=.001
	if(First){
		...
		for (var a = Math.PI*2.5; a >= Math.PI*1.5; a-=.01) {
			var slope = -(Math.cos(a)/Math.sin(a))
			xcord = Math.floor(cX + r * Math.cos(a))
			ycord = Math.floor(cY + r * Math.sin(a))
			Bounce_List.push({xcord,ycord,slope});
...
O.prototype = {
	init: function() {
		...
		this.bounce_Cord_list = [];
		for (var i = Bounce_List.length - 1; i >= 0; i--) {
			var e = Bounce_List[i];
			if(e.xcord == Math.floor(this.x)){
				this.bounce_Cord_list.push(e);
			}
		}
	...
	update: function() {
		if(this.y < this.hit){
			...
			for (var i = this.bounce_Cord_list.length - 1; i >= 0; i--) {
				var e = this.bounce_Cord_list[i];
				if(e.ycord-2 < this.y && e.ycord+2 > this.y){
					this.bounce_direction = e.slope;
...
// console.log(Bounce_List.length);
// for (var i = Bounce_List.length - 1; i >= 30000; i--) {
// 	var e = Bounce_List[i];
// 	if(e.xcord == 693){
// 		console.log(e.xcord, e.ycord);
// 	}
// }
